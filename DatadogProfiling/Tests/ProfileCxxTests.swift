...
final class ProfileCxxTests: XCTestCase {
    ...
    func testProfileAggregation_withIdenticalTraces_aggregatesCorrectly() throws {
        ...
        let totalSampleValue = (0..<5).reduce(0) { sum, i in
            let sample = unpackedProfile.pointee.sample[i]
    ...
    func testProfileAggregation_withZeroAddresses_handlesGracefully() {
        // Given
        let profile = dd_pprof_create(10_000_000)
        defer { dd_pprof_destroy(profile) }
        XCTAssertNotNil(profile)
    ...
    func testProfileAggregation_withMaxStackDepth_handlesCorrectly() throws {
        // Given
        let profile = dd_pprof_create(10_000_000)
        defer { dd_pprof_destroy(profile) }
        XCTAssertNotNil(profile)
...
// MARK: - Test Extensions for C++ Integration

extension ProfileCxxTests {
    func testDirectCxxProfileAccess_createAndManipulate() {
        // Given - This tests direct C++ class access when available
        let profile = dd_pprof_create(10_000_000)
        defer { dd_pprof_destroy(profile) }
        XCTAssertNotNil(profile)

        // When - Add samples via C API
        let addresses: [UInt64] = [0x100001000, 0x100002000]
        let trace = UnsafeMutablePointer<stack_trace_t>.allocate(capacity: 1)
        trace.pointee = .mockWith(tid: 1, addresses: addresses)
        defer { dd_free(trace) }

        dd_pprof_add_samples(profile, trace, 1)

        // Then - Verify through serialization
        var data: UnsafeMutablePointer<UInt8>?
        let size = dd_pprof_serialize(profile, &data)
        defer { dd_pprof_free_serialized_data(data) }
        ...
        // Validate basic profile structure
        let unpackedProfile = perftools__profiles__profile__unpack(nil, size, data)
        defer { perftools__profiles__profile__free_unpacked(unpackedProfile, nil) }
        XCTAssertNotNil(unpackedProfile, "Should be able to unpack profile")
    }
}
